...
"snippet" : "kubernetes.namespace.pod.running.count counts the number of all the running pods in a Namespace. The value is not the sum of (statefulset|daemonset|deployment).pod.running.count , because the metric takes free pods also into account, that is, pods that are not belonging to any controller. Metric Typ...", 
"body" : "kubernetes.namespace.pod.running.count counts the number of all the running pods in a Namespace. The value is not the sum of (statefulset|daemonset|deployment).pod.running.count , because the metric takes free pods also into account, that is, pods that are not belonging to any controller. Metric Type: Gauge Segmented by: Namespace " }, 
...
"snippet" : "In environments where Sysdig Platform is available through an Identity Provider (IDP) on two DNS records (split DNS), one set of users may not be able to log in to Sysdig, as the IDP redirects them only to a single DNS. In order to redirect users to the original DNS name they have requested, configu...", 
...
"snippet" : "Your AWS account must be integrated with Sysdig, but the CloudWatch feature is not required to be enabled. See AWS: Integrate AWS Account and CloudWatch Metrics (Optional) AWS: Integrate AWS Account and CloudWatch Metrics (Optional) Have your S3 bucket name available....", 
"body" : "Your AWS account must be integrated with Sysdig, but the CloudWatch feature is not required to be enabled. See AWS: Integrate AWS Account and CloudWatch Metrics (Optional) AWS: Integrate AWS Account and CloudWatch Metrics (Optional) Have your S3 bucket name available. " }, 
...
"url" : "storage--configure-options-for-capture-files.html#UUID-56022a85-9335-796c-6cc9-dbf4f5094b4b_section-idm4616737381774431618746540322", 
"breadcrumbs" : "Staging Sysdig Documentation \/ Sysdig Platform \/ On-Premises Deployments \/ Storage: Configure Options for Capture Files \/ Configure Custom S3 Endpoint \/ Configure Storage ", 
"snippet" : "Run the following API request to configure storage: POST {{host}}\/api\/filestorage\/config X-Sysdig-Product: SDC Content-Type: application\/json Authorization: Bearer {{API_TOKEN}} { \"name\": \"{STORAGE_NAME}\", \"storageType\": \"S3\", \"config\": { \"bucket\": \"{BUCKET_NAME}\", \"awsAccessKey\": null, \"enabledSSE\"...", 
"body" : "Run the following API request to configure storage: POST {{host}}\/api\/filestorage\/config\nX-Sysdig-Product: SDC\nContent-Type: application\/json\nAuthorization: Bearer {{API_TOKEN}}\n\n{\n \"name\": \"{STORAGE_NAME}\",\n \"storageType\": \"S3\",\n \"config\": {\n \"bucket\": \"{BUCKET_NAME}\",\n \"awsAccessKey\": null,\n \"enabledSSE\": \"false\",\n \"awsSecretKey\": null,\n \"providerKeyId\": null\n },\n \"providedStorage\": true,\n \"enabled\": true\n} " }, 
