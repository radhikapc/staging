...
"body" : "You can set up a custom Amazon S3 compatible storage, such as Minio or IBM Cloud Object Storage , for storing Captures in a Sysdig on-premises deployment. The capture storage location can be used for both Sysdig Monitor and Sysdig Secure. This is an API-only functionality and currently, no UI support is available. You must configure Sysdig Installer file corresponding to your Kubernetes installation in order for this configuration to work. " }, 
{ "title" : "Prerequisites ", 
...
"snippet" : "Ensure that AWS-client compatible credentials used for authentication are present in the environment. Ensure that the list, get, and put operations are functional on the S3 bucket that you wish to use. Confirm this by using the S3 native tools, for example, as described in AWS CLI for IBM Cloud....", 
"body" : "Ensure that AWS-client compatible credentials used for authentication are present in the environment. Ensure that the list, get, and put operations are functional on the S3 bucket that you wish to use. Confirm this by using the S3 native tools, for example, as described in AWS CLI for IBM Cloud. " }, 
